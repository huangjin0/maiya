...
- (void)setUpdatedTime:(NSDate *)updatedTime
{
    ...
    if (updatedTime) {
        ...
        if ([cmp1 day] == [cmp2 day]) { // 今天
            if ([CURRENT_APP_LANGUAGE isEqualToString:@"zh-Hans"]) {
        ...
        // 3.显示日期
        self.updatedTimeLabel.text = [NSString stringWithFormat:CUSTOMLOCAL_STRING_REFRESH(@"Pull_time_tip"), time];
    } else {
        self.updatedTimeLabel.text = CUSTOMLOCAL_STRING_REFRESH(@"Pull_time_no_record");
    }
}
-(void)refreshTime{
    NSDate *updatedTime = [[NSUserDefaults standardUserDefaults] objectForKey:self.dateKey];
    if(updatedTime){
        // 1.获得年月日
        NSCalendar *calendar = [NSCalendar currentCalendar];
        NSUInteger unitFlags = NSCalendarUnitYear| NSCalendarUnitMonth | NSCalendarUnitDay |NSCalendarUnitHour |NSCalendarUnitMinute;
