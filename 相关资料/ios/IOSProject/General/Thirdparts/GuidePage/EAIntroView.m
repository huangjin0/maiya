...
//    [self addSubview:self.pageControl];
    ...
    self.skipButton = [[UIButton alloc] initWithFrame:CGRectMake(self.scrollView.frame.size.width / 2 - 60, self.pageControl.frame.origin.y - 130, 120, self.pageControl.frame.size.height + 100)];
//    self.skipButton.backgroundColor = [UIColor blackColor];
    ...
    [self.skipButton setTitleColor:[UIColor darkTextColor] forState:UIControlStateNormal];
    [self.skipButton addTarget:self action:@selector(skipIntroduction) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:self.skipButton];
    
    self.skipButtonLeft = [[UIButton alloc] initWithFrame:CGRectMake(ScreenWidth - 80, 20, 60, 30)];
    [self.skipButtonLeft setAutoresizingMask: UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin];
    [self.skipButtonLeft setTitle:NSLocalizedString(@"跳过", nil) forState:UIControlStateNormal];
    [self.skipButtonLeft setTitleColor:[UIColor darkTextColor] forState:UIControlStateNormal];
    [self.skipButtonLeft addTarget:self action:@selector(skipIntroductionLeft) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:self.skipButtonLeft];
}
...
//        LastPageIndex = self.pageControl.currentPage - 1;
//        [self makePanelVisibleAtIndex:(NSInteger)self.currentPageIndex - 1];
    } else {
...
- (void)setSkipButtonLeft:(UIButton *)skipButtonLeft {
    [_skipButtonLeft removeFromSuperview];
    _skipButtonLeft = skipButtonLeft;
    [_skipButtonLeft addTarget:self action:@selector(skipIntroductionLeft) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:_skipButtonLeft];
}
